...
CREATE schema if NOT exists user_roles;

CREATE TABLE user_roles.custom_role_names (
    role_name text primary key
);
ALTER TABLE user_roles.custom_role_names ENABLE ROW LEVEL SECURITY;
...
INSERT INTO user_roles.custom_role_names (role_name) VALUES
    ('RoleAdmin');
...
ALTER TABLE user_roles.custom_user_roles ENABLE ROW LEVEL SECURITY;
...
CREATE FUNCTION user_roles.user_has_role(_role text) RETURNS boolean
    LANGUAGE SQL SECURITY DEFINER SET search_path = public
AS $$
select exists (select 1 from user_roles.custom_user_roles where user_id = auth.uid() and role = _role);
$$;

-- Match any roles in array for the current user
CREATE FUNCTION user_roles.user_role_in(_roles text[]) RETURNS boolean
    LANGUAGE SQL SECURITY DEFINER STABLE SET search_path = public
AS $$
select exists (select 1 from user_roles.custom_user_roles where user_id = auth.uid() and role = any (_roles));
$$;

-- Match all roles in array for the current user
CREATE FUNCTION user_roles.user_roles_match(_roles text[]) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER STABLE SET search_path = public
AS $$
declare matches int;
begin
    select count(*) into matches from user_roles.custom_user_roles where auth.uid() = user_id and role = any (_roles);
...
CREATE FUNCTION user_roles.get_user_roles() RETURNS text[]
    LANGUAGE plpgsql SECURITY DEFINER STABLE SET search_path = public
AS $$
begin
    return array (select role from user_roles.custom_user_roles where user_id = auth.uid());
end;
$$;
...
CREATE FUNCTION user_roles.custom_roles_update_to_app_metadata() returns trigger
...
begin
    ...
    select array_agg(role) into _roles from user_roles.custom_user_roles where user_id = new.user_id;
...
CREATE TRIGGER on_custom_role_change
    after insert or update or delete on user_roles.custom_user_roles
    for each row execute function user_roles.custom_roles_update_to_app_metadata();
ALTER TABLE user_roles.custom_user_roles DISABLE TRIGGER on_custom_role_change; -- Enable the trigger in the Dashboard or remove this if desired
